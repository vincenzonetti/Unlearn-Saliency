...
for i in range (0,10):
    ...
    for lr in [0.0001, 0.001, 0.01, 0.1]:
        command = f"python main_forget.py --unlearn GA --unlearn_epochs 10 --unlearn_lr {lr} --class_to_replace {i} --model_path {model_path} --save_dir {unlearn_path} --mask_path masks/cifar10_class{i}_mask_0.8.pt --random_prune"
        subprocess.run(command, shell=True, check=True)
        command = f"python main_random.py --unlearn GA --unlearn_epochs 10 --unlearn_lr {lr} --class_to_replace {i} --model_path {model_path} --save_dir {unlearn_path} --mask_path masks/cifar10_class{i}_mask_0.8.pt --random_prune"
        subprocess.run(command, shell=True, check=True)
...
for i in range (0,10):
    print("class:",i)
    for lr in [0.0001, 0.001, 0.01, 0.1]:
        command = f"python main_forget.py --unlearn RL --unlearn_epochs 10 --unlearn_lr {lr} --class_to_replace {i} --model_path {model_path} --save_dir {unlearn_path} --mask_path masks/cifar10_class{i}_mask_0.8.pt --random_prune"
        subprocess.run(command, shell=True, check=True)
        command = f"python main_random.py --unlearn RL --unlearn_epochs 10 --unlearn_lr {lr} --class_to_replace {i} --model_path {model_path} --save_dir {unlearn_path} --mask_path masks/cifar10_class{i}_mask_0.8.pt --random_prune"
        subprocess.run(command, shell=True, check=True)
